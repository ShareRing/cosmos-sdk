...
func (k *Record) GetName() string {
	return k.Name
}

// GetPubKey fetches a public key of the record
func (k *Record) GetPubKey() cryptotypes.PubKey {
	pk, ok := k.PubKey.GetCachedValue().(cryptotypes.PubKey)
	if !ok {
		panic("unable to cast any to cryptotypes.PubKey")
	}
...
func (k Record) GetAddress() types.AccAddress {
	pk := k.GetPubKey()
